...
def show_pdf_content():
    ...
    try:
        ...
        # Search 2: Get all chunks and look for recent ones
        print(f"\n🔍 Search 2: Looking through all chunks...")
        ...
        if recent_chunks:
            ...
            # If no chunks found, search FAISS directly for any content with "tmp" in filename
            print(f"\n🔍 Search 3: Direct FAISS search for temporary files...")
            query_embedding = embedder.embed_text("temporary file content")
            search_results = faiss_store.search_with_metadata(query_embedding, k=20)
            
            temp_results = []
            for result in search_results:
                filename = result.get('filename', result.get('source', ''))
                if 'tmp' in str(filename).lower():
                    temp_results.append(result)
            
            if temp_results:
                print(f"   ✅ Found {len(temp_results)} temporary file results")
                for i, result in enumerate(temp_results[:3], 1):
                    ...
                    if len(content) > 400:
                        print(f"      ... (showing first 400 of {len(content)} characters)")
                    print(f"      {'-' * 40}")
            else:
                print(f"   ❌ No temporary file results found")
