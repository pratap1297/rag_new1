#!/usr/bin/env python3
"""
Final Core RAG Functionality Test Suite
...
"""
        ...
        try:
            ...
            self.uploaded_doc_ids.append(doc_info['file_id'])
...
"""
        ...
        try:
            # First, find and delete the original document by searching all documents
            print("🗑️ Finding and deleting original document...")
            all_docs = self.get_all_documents()
            original_doc_id = None
            
            for doc in all_docs:
                if doc.get('title') == 'Project Zephyr-7X Protocol':
                    original_doc_id = doc.get('doc_id')
                    break
            
            if not original_doc_id:
                # Try to find by file_id if title search fails
                for doc in all_docs:
                    if doc.get('file_id') == original_doc_info['file_id']:
                        original_doc_id = doc.get('doc_id')
                        break
            ...
            if original_doc_id:
                ...
                print("   Warning: Original document not found, proceeding with upload")
            ...
            # Store the uploaded document info
            self.uploaded_doc_ids.append(updated_doc_info['file_id'])
    ...
    def test_case_3_delete_and_verify_removal(self, doc_info):
        ...
        try:
            ...
            print("🗑️ Finding and deleting document...")
            all_docs = self.get_all_documents()
            doc_id = None
            
            for doc in all_docs:
                if doc.get('title') == 'Project Zephyr-7X Protocol Updated':
            ...
            if not doc_id:
                # Try to find by file_id if title search fails
                for doc in all_docs:
                    if doc.get('file_id') == doc_info['file_id']:
                        doc_id = doc.get('doc_id')
                        break
            ...
            if doc_id:
                ...
                raise Exception("Document not found for deletion")
    ...
    def cleanup_test_documents(self):
        """Clean up any test documents that were uploaded"""
        ...
        try:
            ...
            for doc in all_docs:
                if doc.get('file_id') in self.uploaded_doc_ids:
                    doc_id = doc.get('doc_id')
                    if doc_id and self.delete_document(doc_id):
                        deleted_count += 1
                        print(f"   Deleted: {doc.get('title', 'Unknown')}")
